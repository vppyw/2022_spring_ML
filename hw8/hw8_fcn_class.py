...
class multi_autoencoder(nn.Module):
    ...
    def forward(self, x, fake=False):
        if not fake:
            ...
            img = self.decoder(linear_out).reshape((bs, 3, 64, 64))
        else:
            bs = x.size(0)
            img = self.decoder(torch.randn((bs, 4)).to(device)).reshape((bs, 3, 64, 64))
        return img
