...
class TrainerGAN():
    ...
    def prepare_environment(self):
        ...
        # new dataset from https://www.kaggle.com/datasets/splcher/animefacedataset
    ...
    def inference(self, G_path=None, D_path=None, n_generate=1000, n_output=30, show=False):
        ...
        while True:
            ...
            logits = self.D(imgs)
            for acc, img in zip(logis, imgs):
                if acc > self.config["d_thershold"]:
                    # torchvision.utils.save_image(img, os.path.join(self.output_dir, f'{count}.jpg'))
                    count += 1
                if count == n_generate - 1:
                    break
            if count == n_generate - 1:
                break
