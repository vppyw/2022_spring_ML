...
class TrainerGAN():
    ...
    def gp_sl(self):
        return -self.config["sl_delta"] * math.atan(self.config["sl_slp"] * (self.steps - self.config["sl_dx"])) + self.config["sl_dy"]
    ...
    def gp(self, r_imgs, f_imgs):
        ...
        return ((gd.norm(2, dim=1) - 1) ** 2).mean() * self.gp_sl()
